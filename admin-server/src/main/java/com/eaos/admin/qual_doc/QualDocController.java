...
public class QualDocController {
  ...
  public R<?> uploadGuide(
      ...
      throws Exception {
    return R.ok(
        service.parseAndCreateGuide(
            owner(user), qualificationType, guideName, version, sourceUrl, file));
  }
  ...
  public R<?> uploadMaterial(
      @RequestPart MultipartFile file, @AuthenticationPrincipal LoginUser user) throws Exception {
  ...
  @PostMapping("/tasks")
  public R<?> createTask(
      @Valid @RequestBody QualDto.CreateTaskRequest request,
      @AuthenticationPrincipal LoginUser user) {
  ...
  public R<?> saveDocument(
      @PathVariable String documentId,
      @RequestBody QualDto.SaveDocumentRequest request,
      @AuthenticationPrincipal LoginUser user) {
  ...
  public R<?> rollback(
      @PathVariable String documentId,
      @RequestBody QualDto.RollbackRequest request,
      @AuthenticationPrincipal LoginUser user) {
    return R.ok(
        service.rollbackDocument(documentId, owner(user), request.versionNo(), username(user)));
  }
  ...
  public void export(
    ...
    response.setHeader(
        ...
        ContentDisposition.attachment()
            .filename("qualification-draft.docx", StandardCharsets.UTF_8)
            .build()
            .toString());
    try {
