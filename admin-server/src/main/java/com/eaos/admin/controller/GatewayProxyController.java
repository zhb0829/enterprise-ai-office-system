...
/**
 * 网关反向代理：将业务/AI 接口原样转发给 Python AI 服务（ai-server）。 认证与权限在 Spring Security
 * 层完成，此处仅做协议级转发（方法/路径/查询串/请求体/关键请求头透传，响应流式返回）。
 */
@Slf4j
@RestController
...
public class GatewayProxyController {
  ...
  public void proxy(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    try {
      ...
      log.error("网关转发失败 {} {}: {}", request.getMethod(), path, ex.getMessage());
      response.setStatus(HttpServletResponse.SC_BAD_GATEWAY);
      response.setContentType(MediaType.APPLICATION_JSON_VALUE);
      response.setCharacterEncoding(StandardCharsets.UTF_8.name());
      response.getWriter().write("{\"code\":502,\"message\":\"AI 服务暂时不可用，请稍后重试\",\"data\":null}");
    }
  }
