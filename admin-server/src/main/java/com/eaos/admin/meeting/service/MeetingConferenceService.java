...
public class MeetingConferenceService {
    ...
    public Map<String, Object> page(MeetingPageQuery query) {
        ...
        int pageSize = Math.min(100, Math.max(1, query.getPageSize() == null ? 20 : query.getPageSize()));
        ...
        if (query.getKeyword() != null && !query.getKeyword().isBlank()) {
            ...
            wrapper.and(w -> w.like(Conference::getName, keyword)
                    .or().like(Conference::getOrganizer, keyword)
                    .or().like(Conference::getKeywords, keyword));
        }
        long total = conferenceMapper.selectCount(wrapper);
        wrapper.orderByDesc(Conference::getStartTime).orderByDesc(Conference::getCreatedAt)
                .last("LIMIT " + pageSize + " OFFSET " + ((page - 1) * pageSize));
        List<Map<String, Object>> items = conferenceMapper.selectList(wrapper).stream()
                .map(this::summary)
                .toList();
    ...
    public Map<String, Object> detail(Long id) {
        ...
        List<ConferenceReport> reports = reportMapper.selectList(Wrappers.lambdaQuery(ConferenceReport.class)
        ...
        for (ConferenceReport report : reports) {
            cardsByReport.put(report.getId(), cardMapper.selectList(
                    Wrappers.lambdaQuery(KnowledgeCard.class)
                            .eq(KnowledgeCard::getReportId, report.getId())
                            .orderByAsc(KnowledgeCard::getId)));
        }
        List<KnowledgeCard> cards = latest == null ? List.of() : cardsByReport.getOrDefault(latest.getId(), List.of());
        Map<String, Object> result = new LinkedHashMap<>();
        result.put("conference", conference);
        result.put("materials", materialMapper.selectList(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, id).orderByDesc(ConferenceMaterial::getCreatedAt)));
        result.put("reports", reports);
        result.put("latestReport", latest);
        result.put("cards", cards);
        result.put("cardsByReport", cardsByReport);
        result.put("media", mediaMapper.selectList(Wrappers.lambdaQuery(ConferenceMedia.class)
                .eq(ConferenceMedia::getConferenceId, id).orderByDesc(ConferenceMedia::getPublishedAt)
                .orderByDesc(ConferenceMedia::getCollectedAt)));
        result.put("tasks", taskMapper.selectList(Wrappers.lambdaQuery(ConferenceTask.class)
                .eq(ConferenceTask::getConferenceId, id).orderByDesc(ConferenceTask::getCreatedAt)));
    ...
    public void delete(Long id) {
        ...
        List<ConferenceMaterial> materials = materialMapper.selectList(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, id));
        materials.forEach(material -> storageService.delete(material.getFilePath()));
        List<Long> reportIds = reportMapper.selectList(Wrappers.lambdaQuery(ConferenceReport.class)
                        .eq(ConferenceReport::getConferenceId, id))
                .stream().map(ConferenceReport::getId).toList();
        if (!reportIds.isEmpty()) {
            cardMapper.delete(Wrappers.lambdaQuery(KnowledgeCard.class).in(KnowledgeCard::getReportId, reportIds));
        }
        mediaMapper.delete(Wrappers.lambdaQuery(ConferenceMedia.class).eq(ConferenceMedia::getConferenceId, id));
        taskMapper.delete(Wrappers.lambdaQuery(ConferenceTask.class).eq(ConferenceTask::getConferenceId, id));
        reportMapper.delete(Wrappers.lambdaQuery(ConferenceReport.class).eq(ConferenceReport::getConferenceId, id));
        materialMapper.delete(Wrappers.lambdaQuery(ConferenceMaterial.class).eq(ConferenceMaterial::getConferenceId, id));
    ...
    public ConferenceMaterial addLink(Long conferenceId, MaterialLinkRequest request) {
        ...
        ConferenceMaterial material = baseMaterial(conferenceId, "link",
                blankTo(request.getTitle(), url), "text/html", 0L);
    ...
    @Transactional
    public ConferenceMaterial addTranscript(Long conferenceId, TranscriptRequest request) throws IOException {
        Conference conference = requireManageable(conferenceId);
        ensureEditable(conference);
        ensureMaterialCapacity(conferenceId);
        ...
        if (materialMapper.selectCount(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conferenceId)
                .eq(ConferenceMaterial::getFileSha256, hash)) > 0) {
            storageService.delete(path);
            throw new IllegalArgumentException("该转录文本已添加，请勿重复提交");
        }
        ConferenceMaterial material = baseMaterial(conferenceId, "transcript", title, "text/plain", (long) bytes.length);
    ...
    public ConferenceMaterial upload(Long conferenceId, MultipartFile file) throws IOException {
        ...
        MeetingStorageService.StoredFile stored = storageService.save(conferenceId, file);
        if (materialMapper.selectCount(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conferenceId)
                .eq(ConferenceMaterial::getFileSha256, stored.sha256())) > 0) {
            storageService.delete(stored.path());
            throw new IllegalArgumentException("该文件已上传，请勿重复提交");
        }
        ConferenceMaterial material = baseMaterial(conferenceId, type,
                blankTo(file.getOriginalFilename(), "未命名资料"), mime, file.getSize());
    ...
    public ConferenceTask retry(Long conferenceId, Long taskId) {
        ...
        if (task == null || !conferenceId.equals(task.getConferenceId()) || !"failed".equals(task.getStatus())) {
    ...
    private Map<String, Object> summary(Conference conference) {
        ...
        item.put("materialCount", materialMapper.selectCount(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conference.getId())));
        ConferenceReport latest = reportMapper.selectOne(Wrappers.lambdaQuery(ConferenceReport.class)
                .eq(ConferenceReport::getConferenceId, conference.getId())
                .orderByDesc(ConferenceReport::getVersion).last("LIMIT 1"));
        item.put("latestVersion", latest == null ? null : latest.getVersion());
        return item;
    }

    private ConferenceMaterial baseMaterial(Long conferenceId, String type, String title, String mime, Long size) {
    ...
    private List<ConferenceMaterial> materials(Long conferenceId) {
        return materialMapper.selectList(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conferenceId)
    ...
    private void ensureUniqueUrl(Long conferenceId, String url) {
        if (materialMapper.selectCount(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conferenceId)
                .eq(ConferenceMaterial::getSourceUrl, url)) > 0) {
    ...
    private void ensureMaterialCapacity(Long conferenceId) {
        if (materialMapper.selectCount(Wrappers.lambdaQuery(ConferenceMaterial.class)
                .eq(ConferenceMaterial::getConferenceId, conferenceId)) >= MAX_MATERIALS) {
    ...
    private void requireAdminOrOpenDevelopment() {
        if (securityEnabled && !CurrentUser.isAdmin()) {
