...
public class OpinionAlertService {
    ...
    private static final Set<String> STATES = Set.of("triggered", "acknowledged", "processing", "resolved", "closed");
    ...
    public OpinionAlertEvent handle(Long id, AlertHandleRequest request) {
        ...
        event.setOwner(request.getOwner() == null || request.getOwner().isBlank()
                ? OpinionCurrentUser.username() : request.getOwner());
        ...
        auditService.record("ALERT_HANDLE", "opinion_alert_event", id,
                Map.of("state", state, "note", request.getNote()), null);
        return event;
    }
