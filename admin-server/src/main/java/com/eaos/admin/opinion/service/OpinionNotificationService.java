...
public class OpinionNotificationService {
    ...
    public void notifyAlert(OpinionAlertEvent alert, Long monitorId, Map<String, Object> stats) {
        ...
        String content = String.format("%s %s 舆情告警: %s (触发 %s 次)",
        ...
        if (webhookUrl != null && !webhookUrl.isBlank()) {
            OpinionNotification notification = createNotification(
                    alert.getId(), "webhook", webhookUrl, webhookPayload(alert, name).toString(),
                    "pending", null);
    ...
    public void retryFailedNotifications() {
        List<OpinionNotification> due = notificationMapper.selectList(
                Wrappers.<OpinionNotification>lambdaQuery()
                        .in(OpinionNotification::getStatus, "failed", "pending")
                        .lt(OpinionNotification::getRetryCount, Math.max(1, properties.getNotificationMaxRetries()))
                        .and(w -> w.isNull(OpinionNotification::getNextRetryAt)
                                .or().le(OpinionNotification::getNextRetryAt, LocalDateTime.now()))
    ...
    private void deliverWebhook(OpinionNotification notification) {
        if (!"webhook".equalsIgnoreCase(notification.getChannel())
                || notification.getTarget() == null || notification.getTarget().isBlank()) {
            return;
        }
        ...
        try {
            restTemplate.postForObject(notification.getTarget(),
    ...
    private String statsSummary(Map<String, Object> stats) {
        ...
        return String.format("负面 %s / %s, 比例 %s, 来源 %s, 增长 %s",
                stats.get("negativeCount"), stats.get("totalCount"),
                stats.get("negativeRatio"), stats.get("sourceCount"), stats.get("growthRate"));
    }

    private OpinionNotification createNotification(Long alertId, String channel, String target,
