...
public class OpinionResponseCaseService {
  ...
  public List<OpinionResponseCase> list(String eventType, String riskLevel, String keyword) {
    ...
    if (keyword != null && !keyword.isBlank()) {
      wrapper.and(
          w ->
              w.like(OpinionResponseCase::getTitle, keyword)
                  .or()
                  .like(OpinionResponseCase::getContent, keyword));
    }
  ...
  public List<OpinionResponseCase> listAll() {
    return caseMapper.selectList(
        Wrappers.<OpinionResponseCase>lambdaQuery().orderByDesc(OpinionResponseCase::getId));
  }
  ...
  public Map<String, Object> reindex() {
    ...
    try {
      ...
      Map<String, Object> response =
          restTemplate.postForObject(url, new HttpEntity<>(Map.of(), headers), Map.class);
      if (response == null) {
        throw new IllegalArgumentException("案例索引重建接口无返回");
      }
      auditService.record(
          "CASE_REINDEX",
          "opinion_response_case",
          null,
          Map.of("chunks", response.get("chunks")),
          null);
      return response;
    } catch (Exception e) {
