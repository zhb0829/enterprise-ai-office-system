...
public class OpinionAlertRuleService {
  ...
  public OpinionAlertRule create(AlertRuleRequest request) {
    ...
    auditService.record(
        ...
        "opinion_alert_rule",
        rule.getId(),
        Map.of("name", rule.getName(), "monitorId", rule.getMonitorId()),
        null);
    return rule;
  }
  ...
  public OpinionAlertRule update(Long id, AlertRuleRequest request) {
    ...
    auditService.record(
        "ALERT_RULE_UPDATE", "opinion_alert_rule", id, Map.of("name", rule.getName()), null);
    return rule;
  }
  ...
  private void apply(OpinionAlertRule rule, AlertRuleRequest request) {
    ...
    rule.setCooldownMinutes(
        request.getCooldownMinutes() == null ? 60 : request.getCooldownMinutes());
